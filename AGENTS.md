...
`Version: Unreleased` is a `factorix` convention for the not-yet-released section, not part of the Factorio changelog format. The release workflow renames it to the released version and does not open a new one, so between releases `changelog.txt` starts with the last released version. Add a fresh `Unreleased` section at the top for the first user-visible change of a new cycle; put later entries in that same section.
